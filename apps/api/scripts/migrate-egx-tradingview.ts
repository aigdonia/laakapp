/**
 * One-time migration: replace all EGX stocks with TradingView data.
 *
 * Usage:
 *   npx tsx scripts/migrate-egx-tradingview.ts \
...
async function fetchTradingViewEGX(): Promise<TVStock[]> {
  console.log("[tv] Fetching EGX stocks from TradingView scanner...");
...
async function main() {
  ...
  // 1. Fetch existing EGX stocks from DB
  console.log("[migrate] Step 1: Fetching existing EGX stocks...");
  const existing = await apiGet<{
    data: Array<{ id: string; symbol: string }>;
  }>("/stocks?countries=EG&limit=500");
  console.log(`[migrate] Found ${existing.data.length} existing EGX stocks`);
  ...
  if (existing.data.length > 0) {
    ...
    const egxCompliance = compliance.filter((c) => stockIds.has(c.stockId));
    for (const record of egxCompliance) {
      await apiDelete(`/stock-compliance/${record.id}`);
    }
    console.log(`[migrate]   Deleted ${egxCompliance.length} compliance records`);

    // Delete financials (API returns plain array)
    const financials = await apiGet<
      Array<{ id: string; stockId: string }>
    >("/stock-financials?limit=5000");
    const egxFinancials = financials.filter((f) => stockIds.has(f.stockId));
    for (const record of egxFinancials) {
      await apiDelete(`/stock-financials/${record.id}`);
    }
    console.log(`[migrate]   Deleted ${egxFinancials.length} financial records`);

    // Delete stocks
    console.log("[migrate] Step 3: Deleting existing EGX stocks...");
  ...
  const tvStocks = await fetchTradingViewEGX();
